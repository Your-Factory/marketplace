...
create unique index image_formats_name_uindex
    on image_formats (name);
...
create or replace function check_user_authentication(u_login text, u_password_hash text) returns integer
    immutable
    language plpgsql
as
$$
...
create or replace procedure add_images_to_model(model_id integer, m_images bytea[], m_images_formats text[])
    language plpgsql
as
$$
DECLARE
    ...
    formats_id   int[];
BEGIN
    select array_agg(model_formats.id)
    from model_formats
    where model_formats.name = ANY (m_images_formats)
    into formats_id;
    for image_number in 1 .. array_length(m_images, 1)
        loop
...
create or replace procedure add_model(m_name text, m_description text, m_model_file bytea, m_author_id integer,
                                      m_model_format text, m_images bytea[], m_images_formats text[])
    language plpgsql
as
$$
DECLARE
...
create or replace procedure add_model_information(m_name text, m_description text, m_model_file bytea,
